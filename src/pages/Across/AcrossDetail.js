...
export default class AcrossDetail extends React.Component {
  ...
  async getUnhandleApplicationDetail(taskId) {
    try {
      ...
      const result = await Request.post(`oppo/leaderApproval`,{
  ...
  async getApplicationDetail(bizkey) {
    try {
      const result = await Request.post(`oppo/getWorkbenchOppoDetail`, {
        bizkey:bizkey
      });
      const {attachList,clAttachList, jtAttachStrList, kjAttachList,oppoProcess,zsAttachList} = result;
        ...
        const mark = oppoProcess.mark
        let markjson = null;
        if(mark != null && mark != '') {
          markjson = JSON.parse(mark)
        }
        this.setState({
          oppoMessage:oppoMessage,
          tasks:tasks,
  ...
  _submit(item,gooutFlow) {
    if(!gooutFlow) {
      ...
      } else if(this.state.currentTask == '集团派单' || this.state.currentTask == '领导审批' || this.state.currentTask == '接口人分派') {
        let isOk = item == 'back'?false:true;
        this.props.navigation.navigate('AcrossHandle', {
          data: {
            isOk,
      ...
      <SafeAreaView forceInset={{ top: 'never' }} style={{ flex: 1, backgroundColor: Colors.backgroundV2}}>
          ...
          <View style={styles.container}>
            ...
            <View style={{marginBottom:px(45)}}>
              ...
              <ListItem
                ...
                <ListItem title="所属行业" content={this.state.oppoMessage.industry1 + "," + this.state.oppoMessage.industry2 + "," + this.state.oppoMessage.industry3} />
                ...
                <ListItem title="业务类型" content={this.state.oppoMessage.Keywords} />
                <ListItem title="客户需求简介" isColumn = {true} content={this.state.oppoMessage.custReq}></ListItem>
                <ListItem
                    isLast={true} 
                    isColumn = {true} 
                    title="接口人信息"
                  >
                    <Profile
                      name={this.state.oppoMessage.inf_user}
                      avatar={null}
                      avatarName = {this.state.oppoMessage.inf_user}
                      describe={this.state.oppoMessage.inf_phone}
                      describe2={this.state.oppoMessage.interfacepeople}
                    />
                </ListItem>
            </View>
            </View>
            ...
            <View style={{marginBottom:px(45)}}>
            <Text style={[styles.title,{fontSize:px(32)}]}>{this.state.oppoMessage.productType == '5G' &&'需求类别：'}{this.state.oppoMessage.productType}</Text>
            {this.state.oppoMessage.productType != '5G' && 
              <View style={styles.card}>
            ...
            <View style={{flexDirection:'row',justifyContent:'space-around',marginBottom:px(30)}}>
              <TouchableOpacity onPress={() => this.setState({logtype:1})} style={[{fontSize:px(30)},this.state.logtype==1 ? {paddingBottom:px(10),color:'#33a6fa',borderBottomColor:'#33a6fa',borderBottomWidth:px(4)} : {color:'#000000'}]}><Text style={[ {fontSize:px(30)},this.state.logtype==1 ? {color:'#33a6fa'} : {color:'#000000'}]}>支撑记录</Text></TouchableOpacity>
              <TouchableOpacity onPress={() => this.setState({logtype:2})} style={[{fontSize:px(30)},this.state.logtype==2 ? {paddingBottom:px(10),color:'#33a6fa',borderBottomColor:'#33a6fa',borderBottomWidth:px(4)} : {color:'#000000'}]}><Text style={[ {fontSize:px(30)},this.state.logtype==2 ? {color:'#33a6fa'} : {color:'#000000'}]}>费用凭证</Text></TouchableOpacity>
              <TouchableOpacity onPress={() => this.setState({logtype:3})} style={[{fontSize:px(30)},this.state.logtype==3 ? {paddingBottom:px(10),color:'#33a6fa',borderBottomColor:'#33a6fa',borderBottomWidth:px(4)} : {color:'#000000'}]}><Text style={[ {fontSize:px(30)},this.state.logtype==3 ? {color:'#33a6fa'} : {color:'#000000'}]}>其他信息</Text></TouchableOpacity>
        </View>
        ...
        <View style={this.state.logtype==1 ? {display:'flex'}:{display:'none'}}>
          <View style={{marginBottom:px(45)}}>
          {
            ...
            <View>
                  {this.state.supptDetail.feedbackList.map((item, index) => {
                    return (
                      <View style={[styles.card,{paddingLeft:0,marginBottom: this.state.supptDetail.feedbackList.length === index + 1 ? 0 : px(30)}]}>
                      ...
                      {(item.attachList != null && item.attachList.length > 0) &&
                    <View style={styles.card}>
                  {item.attachList.map((attatch, attachindex) => {
                  let avatar = null;
                  const nameNow = attatch.nameCh && attatch.nameCh.split('.').length ? attatch.nameCh.split('.')[attatch.nameCh.split('.').length - 1] : '';
                  switch(nameNow) {
                    case 'docx' :
                      avatar = require('../../images/word.png');
                      break;
                  ...
                  return (
                    <TouchableOpacity onPress={() => this._opendoc(attatch.location, attatch.nameCh, attatch.fileSize, attatch.flowId)} key={attatch.nameCh + attachindex} style={{
                      borderRadius: px(10),
                      backgroundColor: '#fff',
                      marginBottom: item.attachList.length === attachindex + 1 ? 0 : px(30)
                    }}>
                      <Profile
                        isCircle={false}
                        avatar={avatar}
          ...
          <View style={{marginBottom:px(45)}}>
          {
            ...
            <View>
                  {this.state.costlist.map((item, index) => {
                    return (
                      <View style={[styles.card,{paddingLeft:0, marginBottom: this.state.costlist.length === index + 1 ? 0 : px(30)}]}>
                      <View style={{flexDirection:'row',flexWrap:'nowrap',alignItems:'center',paddingBottom:px(22)}}>
                        <View style={styles.logname}><Text style={{marginLeft:px(10),fontSize:px(24),lineHeight:px(40),color:'white'}}>{item.user.name}</Text></View> 
                        <Text style={{marginLeft:px(25),marginRight:px(50),flex:1,fontSize:px(24),color: "#666666"}}>{item.user.orgName}</Text>
...

          {
            ...
            <View>
            <View>
              <Text style={styles.title}>支撑评价</Text>
            </View>
            <View style={{marginBottom:px(45)}}>
            <View style={styles.card}>
            <Comment mark={this.state.mark} diabled={true}></Comment>
            </View>
            </View>
            </View>
          }
        ...
        {
          (this.state.result != null && this.state.oppoMessage != null &&  this.state.isUnhandle && this.state.currentTask === '商机录入') &&
        ...
        {
          (this.state.result != null && this.state.isUnhandle && this.state.currentTask == '领导审批' && this.state.outFlows != null && this.state.outFlows.length == 2) &&
        ...
        {
          (this.state.result != null && this.state.isUnhandle && this.state.currentTask == '集团派单' && this.state.outFlows != null && this.state.outFlows.length == 2) &&
        ...
        {
          (this.state.result != null && this.state.isUnhandle && this.state.currentTask == '接口人分派' && this.state.outFlows != null && this.state.outFlows.length == 2) &&
        ...
        {
          (this.state.result != null && this.state.isUnhandle && this.state.currentTask == '跨省支撑' && this.state.outFlows != null && this.state.outFlows.length == 2) &&
        ...
        {
          (this.state.result != null && this.state.isUnhandle && this.state.currentTask == '支撑确认' && this.state.outFlows != null && this.state.outFlows.length == 2) &&
