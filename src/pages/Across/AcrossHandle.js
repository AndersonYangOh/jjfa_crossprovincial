  ...
  async _submit() {
    ...
    if (remark.length === 0 && !this.state.isOk) {
      Alert.alert(
        ...
        if(currentTask == '集团派单' || this.state.currentTask == '领导审批' || currentTask == '接口人分派') {

          if(currentTask == '接口人分派' && this.state.isOk && this.state.supportid == '') {
            Alert.alert(
              '请选择支撑经理',
              '',
              [
          ...
          try {
            result = await Request.post('oppo/completeTask', {
              ...
              style,
              userId,
              supptUser:this.state.supportid
            })
  ...
  render() {
    ...
    <SafeAreaView forceInset={{ top: 'never' }} style={{ flex: 1, backgroundColor: Colors.background}}>
      ...
      <ScrollView>
        ...
        {(this.state.currentTask == '接口人分派' && this.state.isOk ) &&
          <ListSearchItem

            title = "支撑经理"
            value = {this.state.supportname}
            onSearch={() => this.searchClick('support')}
          />
        }
