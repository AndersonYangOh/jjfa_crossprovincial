...
export default class AcrossApplication extends React.Component {
  ...
  componentWillMount() {
    ...
    const innerdata = navigation.getParam('innerdata',null);
  ...
  async getNext() {
    ...
    if(this.state.innerdata != null) { //省内转跨省
      try {
        const result = await Request.post(`snoppo/createOppo`,{
          orgId:this.state.orgId,
          userId:this.state.userId,
          org2Id:this.state.org2Id,
          provOppoTaskId:this.state.innerdata.provOppoTaskId,
          orderId1:this.state.innerdata.orderId,
          id:this.state.innerdata.id
        });
        const {showAttachStr,industry1,industry2,industry3,users3,users2,users1,user3,source,busin,oppo,order,taskMess,nonkeyHandler} = result;
        if (users1.length === 0) {
           Alert.alert(
            '您暂时无法进行跨省支撑申请',
            '',
        ...
        for(let i = 0;i < keywordsarr.length;i++) {
          ...
          for(let j = 0;j < busin.length;j++) {
            if(busin[j].codeName == keywordsarr[i]) {
              keywordstext += busin[j].codeValue + ','
              break;
            }
          }
        }
        ...
        if(keywordstext.length > 0) {
          keywordstext = keywordstext.substring(0,keywordstext.length - 1)
        }

        this.setState({
          oppoNum: oppo.oppoNumber,
          approverPerson: users1,
          groupHandler:users2,
          yunGuangHandler:users3,
          source:source,
          resourceOriginal: new Map().set(oppo.source,true),
          resourcevalue:oppo.source,
          busin:busin,
          dealer:users2,
          resourceOptions:resourceOptionstmp,
          keywordsOptions:keywordsOptionstmp,

          productDescribe: oppo.oppoName,
          customer: oppo.custName,
          customername: oppo.custContact,
          income:oppo.estimatedIncome + "",
          tel: oppo.contactTelephone,
          managername:user3.name,
          managerId:user3.id,
          ...
          industry1: oppo.industry,
          industry1name:industry1,
          industry2: oppo.industry2,
          industry2name:industry2,
          industry3: oppo.industry3,
          industry3name:industry3,

          gHandler:result['5gHandler'],
          nonkeyHandler:nonkeyHandler
        });

        this.getIndustry23(2,oppo.industry);
        if(oppo.industry2 != '')
          this.getIndustry23(3,oppo.industry2);
      } catch (error) {
      ...
      try {
        const result = await Request.post(`oppo/createOppoNeed`,{
          orgId:this.state.orgId,
          userId:this.state.userId,
          org2Id:this.state.org2Id
        });
        const {oppoNum, approverPerson, groupHandler,yunGuangHandler,source,busin,nonkeyHandler} = result;
  ...
  searchClick =(type) => {
    if(type == 'inter' && !this.innerClickFlag) {
      return;
    }
    let that = this;
      ...
      choose:(item,type) => {
        if(type == 'manager') {
          ...
          if(!!item.name && !!item.email) {
            that.innerClickFlag = false;
            that.setState({
              inter_name:item.name,
              inter_mail:item.email,
            })
          } else {
            that.innerClickFlag = true;
            that.setState({
              inter_name:'点击选择添加',
              inter_mail:'',
            })
  ...
  _changeRadio(value, name) {
    ...
    switch(name) {
      ...
      case 'across':
        let dealertmp = [];
        if(value == 0) {
          dealertmp = this.state.groupHandler;
        } else if(value == 1) {
          dealertmp = this.state.yunGuangHandler;
        } else if(value == 2) {
          dealertmp = this.state.gHandler;
        } else if(value == 3) {
          dealertmp = this.state.nonkeyHandler;
        }
        this.innerClickFlag = true;
        this.setState({
          inter_id:'',
  ...
  async _submit() {
    ...
    ||keywords == '点击添加业务类型' ||keywords == '' || (acrossvalue !=2 && productId==='') || productname === '' || acrossvalue === '' || intro === '' || startDate === '' || endDate===''
    || workload === '' || approverId === '' || dealId === ''||inner_name === '') {
      Alert.alert(
        '请填写完整信息 ',
        '',
        [
      ...
      const req = {
          ...
          projectname1:productId == '' ? '' : productname,
        ...
        <ScrollView scrollEventThrottle = {10} onScroll={this._changeScroll.bind(this)}>
          <View style={detailStyles.container}>
            ...
            <View style={{marginBottom:px(30), paddingVertical: px(20)}}>
              ...
              <ListSearchItem
                title = "接口人"
                value = {this.state.inter_name}
                onChangeText = {(inter) => this.setState({inter})}
                onSearch={() => this.searchClick('inter')}
              />
        ...
        <Modal
          ...
          <View style={{flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent:'center', alignItems:'center'}}>
            <View style={{backgroundColor: '#fff', marginHorizontal: px(30),borderRadius:px(20),  width: px(690)}}>
              ...
              <Radios
                  itemStyle={{width:'30%',justifyContent: 'flex-start',marginTop:px(20)}}
