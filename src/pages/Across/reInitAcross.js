...
export default class reInitAcross extends React.Component {
  ...
  async getNext() {
    try {
      const result = await Request.post(`oppo/editOppoNeed`,{
      ...
      const {approverPerson, attachList, busin,groupHandler,source,taskMess,yunGuangHandler,nonkeyHandler} = result;
      ...
      this.setState({
      inter_id:'',
      inter_name:oppoMessage.inf_user,
      inter_mail:oppoMessage.interfacepeople,
      ...
      dealId: oppoMessage.groupLeaderId,
    ...
    this.getIndustry23(2,oppoMessage.industry1Code);
    if(oppoMessage.industry2Code != '')
      this.getIndustry23(3,oppoMessage.industry2Code);
  ...
  async _submit() {
    ...
    || workload === '' || approverId === '' || dealId === '' || inter_name == '') {
      Alert.alert(
        '请填写完整信息 ',
        '',
        [
